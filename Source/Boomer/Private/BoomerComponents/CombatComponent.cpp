...
void UCombatComponent::SetHUDCrosshairs(float DeltaTime)
{
	...
	if (Controller)
	{
		...
		if (HUD)
		{
			...
			// TODO: Crosshair Spread based on weapon and velocity instead of just velocity? Crouching?
			// [0, 600] -> [0, 1]
			...
			if (bCrosshairOverPlayer)
			{
				CrosshairOverPlayerFactor = FMath::FInterpTo(CrosshairOverPlayerFactor, -0.3, DeltaTime, 20.f);
			}
			else
			{
				CrosshairOverPlayerFactor = FMath::FInterpTo(CrosshairOverPlayerFactor, 0.0f, DeltaTime, 20.f);
			}
...
void UCombatComponent::EquipWeapon(AWeapon* WeaponToEquip)
{
	...
	// Drop weapon if already equipping a weapon
	if (EquippedWeapon)
	{
		EquippedWeapon->Drop();
	}
	...
	// Display weapon ammo and carried ammo on HUD
...
void UCombatComponent::TraceUnderCrosshairs(FHitResult& TraceHitResult)
{
	...
	if (bScreenToWorld)
	{
		...
		if (TraceHitResult.GetActor() && TraceHitResult.GetActor()->Implements<UInteractWithCrosshairsInterface>())
		{
			...
			bCrosshairOverPlayer = true;
		}
		else
		{
			HUDPackage.CrosshairsColor = FLinearColor::White;
			bCrosshairOverPlayer = false;
		}
	}	
}
